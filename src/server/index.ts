...
const UNIX_SOCKET = process.env.AC_UNIX_SOCKET ?? '/tmp/agent-dispatcher.sock'
...
wss.on('connection', (browserWs) => {
  ...
  runnerWs.on('open', () => {
    ...
    for (const msg of pendingToRunner) runnerWs.send(msg)
    pendingToRunner.length = 0
  })
  ...
  // Browser → Runner (buffer until runner socket is open)
  browserWs.on('message', (data) => {
    if (runnerWs.readyState === WebSocket.OPEN) {
      runnerWs.send(data.toString())
    } else {
      pendingToRunner.push(data.toString())
    }
  })
